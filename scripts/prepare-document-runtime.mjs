// Build-time preparation only. App users receive this pinned runtime in the bundle.
...
const sources=path.join(root,'apps/desktop/native/document-helper')
const source=JSON.parse(await readFile(path.join(sources,'runtime-source.json'),'utf8'))
if(`${process.platform}-${process.arch}`!==source.platform)throw Error('This document runtime currently targets Apple silicon macOS only.')
const output=path.join(root,'.local-data/document-helper'),target=path.join(output,'runtime')
...
try{if((await readFile(path.join(target,'source-hash'),'utf8')).trim()===fingerprint){await stat(path.join(target,'python/bin/python3.12'));console.log('Document runtime already prepared.');process.exit(0)}}catch{}
...
try{
 ...
 await command(python,['-I','-m','pip','install','--isolated','--disable-pip-version-check','--no-cache-dir','--target',path.join(stage,'python/lib/python3.12/site-packages'),'--require-hashes','--only-binary',':all:','-r',path.join(sources,'requirements.lock')])
