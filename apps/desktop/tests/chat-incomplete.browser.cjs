...
;(async () => {
 ...
 try {
  ...
  await message.waitFor().catch(error=>{throw Error(error.message+' Page errors: '+JSON.stringify(errors))});await page.evaluate(()=>window.failRun(true));const alert=page.getByRole('alert').filter({hasText:'Response incomplete'});await alert.waitFor();assert.match(await alert.innerText(),/16 tool actions/);assert.match(await alert.innerText(),/content above is partial/);assert.equal(await page.getByRole('button',{name:'Stop response',exact:true}).count(),0);
  const banner=await alert.boundingBox(),thinking=await page.locator('.chat-thinking').boundingBox();assert.ok(banner.y>thinking.y);assert.ok(banner.y>=0&&banner.y+banner.height<900,'failure remains visible at the bottom of a long response');assert.equal(await page.getByText('Working on the document.',{exact:true}).isVisible(),true);const resume=button('Continue response');assert.equal(await resume.isDisabled(),true);assert.equal(await message.inputValue(),'Next request');await message.fill('');await resume.click();assert.match((await page.evaluate(()=>window.events.filter(e=>e[0]==='send').at(-1)))[1].text,/Continue.*partial results/);await page.screenshot({path:'.local-data/chat-incomplete.png'});assert.deepEqual(errors,[]);console.log('PASS incomplete response: alert and continuation below partial content, failed reason visible alongside successful tool steps, generation stopped.')
