...
test('modules access native capabilities through the shared host contract',()=>{
 for(const name of ['hq','notes','tasks','chat'])for(const file of sources(path.join(root,'modules',name))){
...
test('desktop shell does not statically link feature implementations',()=>{
 for(const file of sources(path.join(root,'apps/desktop/src'))){
  const source=fs.readFileSync(file,'utf8');assert.doesNotMatch(source,/from\s+['"][^'"]*(?:modules\/|@zq\/module-(?:chat|notes|tasks|hq))/,file);
 }
 for(const name of ['hq','notes','tasks','chat'])assert.ok(fs.existsSync(path.join(root,'modules',name,'index.tsx')));
});
