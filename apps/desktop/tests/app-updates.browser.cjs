...
(async()=>{
 ...
 try{
  ...
  await page.getByRole('button',{name:'Download update',exact:true}).click();await page.getByRole('navigation',{name:'Settings sections'}).getByRole('button',{name:'Appearance',exact:true}).click();await page.getByRole('button',{name:'Updates',exact:true}).click();await page.getByText('zQ 0.2.0 is ready to install').waitFor();
  await page.getByRole('button',{name:'Restart and install',exact:true}).click();await page.getByRole('dialog').waitFor();await page.getByRole('button',{name:'Later',exact:true}).click();await page.getByRole('dialog').waitFor({state:'hidden'});
  ...
  if(process.env.ZQ_SCREENSHOT_DIR){fs.mkdirSync(process.env.ZQ_SCREENSHOT_DIR,{recursive:true});for(const name of ['updates-light.png','updates-dark.png','updates-narrow.png'])fs.copyFileSync(path.join(directory,name),path.join(process.env.ZQ_SCREENSHOT_DIR,name))}
