...
const host = { workspace: { layout: { view: 'Chat' }, notes: [], tasks: [], settings: {}, profile: {} }, closing: false, registerFlush: () => () => {}, commands: { register: () => () => {}, run: () => true }, navigate: () => {}, notify: message => window.events.push(['notify', message]), openSettings: section => {window.events.push(['settings', section]);window.showSkillSettings?.()}, services: { connectors: { list: () => ok([]), subscribe: () => () => {} }, chat: {
