...
;(async () => {
 ...
 try {
  ...
  await details.getByText('Instructions loaded',{exact:true}).waitFor(); await details.getByText('Selected automatically',{exact:true}).waitFor(); await details.getByText('guide.md',{exact:true}).waitFor(); await details.getByText('Package resources are preserved; scripts and tools are not executed.',{exact:true}).waitFor()
  ...
  await button('Using Writing style').click(); const selected=page.locator('.chat-skill-activity').nth(1); await selected.getByText('From your selection',{exact:true}).waitFor(); assert.equal(await selected.getByText(/scripts and tools/).count(),0)
