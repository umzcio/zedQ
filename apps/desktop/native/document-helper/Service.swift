...
final class ServiceDelegate: NSObject, NSXPCListenerDelegate {
    func listener(_ listener: NSXPCListener, shouldAcceptNewConnection connection: NSXPCConnection) -> Bool {
        ...
        let helper = Helper()
        connection.exportedInterface = NSXPCInterface(with: DocumentHelperProtocol.self)
...
final class Helper: NSObject, DocumentHelperProtocol {
    ...
    func execute(_ request: Data, withReply reply: @escaping (Data) -> Void) {
        guard validRequest(request) else {
            reply(jsonData(["exitCode": 1, "error": "invalid or oversized request", "files": []])); return
        }
        lock.lock()
        if cancelled { lock.unlock(); reply(jsonData(["exitCode": 130, "error": "cancelled", "files": []])); return }
        if busy { lock.unlock(); reply(jsonData(["exitCode": 1, "error": "one active job per connection", "files": []])); return }
    ...
    private func run(_ request: Data, reply: @escaping (Data) -> Void) {
        ...
        var job = manager.temporaryDirectory.appendingPathComponent("zq-document-" + UUID().uuidString, isDirectory: true)
        ...
        do {
            // Caller cancellation closes XPC before its supervisor has necessarily
            // finished cleanup. Wait for that lease rather than failing the next
            // queued document. Admission and execution share one deadline.
            while lease == nil {
                lock.lock(); let wasCancelled = cancelled; lock.unlock()
                if wasCancelled { result = ["exitCode": 130, "error": "cancelled", "files": []]; return }
                if ProcessInfo.processInfo.systemUptime >= deadline { result["error"] = "document helper admission timed out"; return }
                lease = try JobLease.acquire()
                if lease == nil { usleep(10000) }
            }
            job = lease!.jobs.appendingPathComponent("zq-document-" + UUID().uuidString, isDirectory: true)
            ...
            process.executableURL = Bundle.main.bundleURL.appendingPathComponent("Contents/MacOS/DocumentHelperSupervisor")
            process.arguments = [Bundle.main.bundleURL.appendingPathComponent("Contents/MacOS/DocumentHelperWorker").path, resources.path, job.path, String(getpid())]
            ...
            lock.lock(); let wasCancelled = cancelled; lock.unlock()
            result = wasCancelled ? ["exitCode": 130, "error": "cancelled", "files": []] : response!
        } catch {
...
final class BoundedOutput {
    ...
    func append(_ data: Data, isError: Bool) -> Bool {
        lock.lock(); defer { lock.unlock() }
        if stdout.count + stderr.count + data.count > 6 * 1024 * 1024 { overflow = true; return false }
