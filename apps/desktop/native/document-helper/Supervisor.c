// Fixed document worker supervisor. Never evaluates skill code.
#include <sys/event.h>
#include <sys/file.h>
#include <sys/stat.h>
#include <sys/wait.h>
...
int main(int argc, char **argv) {
    ...
    pid_t child = -1;
    // Register against the live parent before launching document work.
    ...
    for (;;) {
        ...
        // App Sandbox cannot inspect another process with proc_pid_rusage.
        // Monitor private job files here; input/output/CPU/wall limits remain bounded.
        if (now >= next_sample) {
