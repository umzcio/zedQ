...
class NativeOAuthProvider {
 constructor({row,secrets,signal,openExternal,onAuthenticating,allowLoopbackHttp}){
  ...
  this.interactive=true;this.nonce=randomBytes(32).toString('base64url');
 ...
 async start(){
  if(this.row.authType==='bearer'){
   ...
   if(!token?.value||token.url!==this.row.url)throw new ConnectorError('Enter a bearer token for this connector endpoint, then connect.');
 ...
 transportAuth(){if(this.row.authType==='bearer')return {token:async()=>this.savedTokens?.access_token,onUnauthorized:async()=>{throw new ConnectorError('Bearer token was rejected or expired. Update the token in connector settings and reconnect.')}};return {token:async()=>this.tokens()?.access_token,onUnauthorized:ctx=>{
 ...
 clientInformation(ctx){if(this.row.clientId)return {client_id:this.row.clientId,...(this.manualSecret?{client_secret:this.manualSecret.value,token_endpoint_auth_method:this.clientAuthMethod}:{token_endpoint_auth_method:'none'}),issuer:ctx?.issuer};return this.savedClient}
