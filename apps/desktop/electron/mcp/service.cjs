...
class ConnectorService {
 ...
 async connect(id,{interactive=true}={}){const row=this.get(id);if(this.locks.has(id)||busy(row))throw new ConnectorError('Connector is busy.');if(row.status==='connected')return clone(row);
  ...
  try{
   ...
   if(LEGACY_GOOGLE[row.catalogId]){
    ...
    const secrets=new SecretStore(this.credentials,id,{interactive}),secret=await secrets.get('clientSecret'),tokens=await secrets.get('tokens'),changes={};
   ...
   if(isBundledGoogle(row)){
    ...
    const bundled=await create({catalogId:row.catalogId,signal:controller.signal,fetchImpl:this.fetchImpl,getToken:force=>googleAccessToken(provider,this.fetchImpl,force),calendarWriteAccess:()=>String(provider.savedTokens?.scope??'').split(/\s+/).includes('https://www.googleapis.com/auth/calendar.events'),driveWriteAccess:()=>String(provider.savedTokens?.scope??'').split(/\s+/).includes('https://www.googleapis.com/auth/drive.file')});Object.assign(session,{client:bundled.client,bundledClose:bundled.close,prepareSend:bundled.prepareSend,prepareCalendarAction:bundled.prepareCalendarAction,prepareDriveUpload:bundled.prepareDriveUpload});
 ...
 async prepareDriveUpload(id,args,file,{signal,expectedRevision}={}){
  const row=this.get(id),session=this.sessions.get(id);
  if(row.catalogId!=='google-drive'||!isBundledGoogle(row)||row.status!=='connected'||!session?.prepareDriveUpload||row.revision!==expectedRevision||!row.tools.some(t=>t.name==='upload_file'&&t.enabled))throw new ConnectorError('Drive uploads are not enabled or its tools changed.');
  const prepared=await session.prepareDriveUpload(args,file,signal);
  if(this.sessions.get(id)!==session||row.revision!==expectedRevision){prepared.discard();throw new ConnectorError('Drive tools changed during review.')}return prepared;
 }
