...
class ConnectorService {
 ...
 async save(input){this.assertOpen();const valid=this.validateInput(input);const existing=input.id?this.get(input.id):undefined;
  ...
  try{
   ...
   const next={...valid,id:row.id,status:row.status,revision:row.revision+(existing&&changed?1:0),tools:changed?[]:row.tools,hasToken:endpointChanged?false:row.hasToken===true,hasClientSecret:secretBindingChanged?false:row.hasClientSecret===true};
   ...
   await secrets.transaction(changes,()=>{if(existing)Object.assign(row,next);else this.rows.push(next);for(const key of ['clientId','clientMetadataUrl','catalogId','redirectPort','redirectHost','error'])if(!(key in next))delete row[key];this.commit()});
