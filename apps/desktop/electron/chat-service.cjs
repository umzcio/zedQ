...
class ChatService{
 ...
 send({conversationId,text:content,noteIds=[],attachmentIds=[],tools:requestedTools=[],artifactContext,skillIds,connectorIds},adapter,revision,queued){
  ...
  const connectorSelection=selectedConnectors(this.connectors,queued?queued.connectorIds??[]:revision?revision.user.connectorIds??[]:connectorIds===undefined?c.connectorIds:connectorIds,project);
  const connectorEntries=connectorTools(this.connectors,connectorSelection);
  ...
  const now=Date.now();const user=revision?revision.user:{id:randomUUID(),connectorIds:connectorSelection,tools,skillContext,...(skillCatalog.length?{skillCatalog}:{}),role:'user',content:content.trim()||'Please review the attached material.',thinking:'',status:'complete',createdAt:now,context,attachments,...(focused?{artifactContext:structuredClone(focused)}:{}),...(project?{projectContext:structuredClone(project)}:{}),error:''};
 ...
 shutdown(){this.connections.assertIdle();this.shuttingDown=true;this.epoch++;try{this.stopAll();clearTimeout(this.publishTimer);this.publishTimer=null;this.persist()}catch(error){this.shuttingDown=false;throw error}}
 resumeAfterWindowClose(){if(!this.shuttingDown)return;require('./chat-queue.cjs').pauseQueuesOnRestart(this.state);this.persist();this.shuttingDown=false}
}
