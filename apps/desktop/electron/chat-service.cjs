...
class ChatService{
 ...
 send({conversationId,text:content,noteIds=[],attachmentIds=[],tools:requestedTools=[],artifactContext,skillIds},adapter,revision,queued){
  ...
  Promise.resolve().then(async()=>{
   ...
   const executeDocument=local&&this.artifacts?createDocumentExecutor({artifacts:this.artifacts,check,update:updateTool,documents,source:{conversationId,messageId:assistant.id,versionId:assistant.versionId,conversationTitle:this.conversation(conversationId).title,...(project?{projectId:project.id,projectName:project.name}:{})}}):async()=>({error:'Document creation is unavailable.'});
