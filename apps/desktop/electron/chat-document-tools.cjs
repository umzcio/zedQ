...
function createDocumentExecutor({artifacts,check,update,source,documents={available:new Map()}}){
 ...
 return async call=>{
  ...
  activity('running',revising?'Revising a document':'Creating a document');
  try{
   ...
   const output=await artifacts.render(input);
   check();
   if(revising)artifacts.checkRevision(input.artifactId,expected,input.format);
