...
const DOCUMENT_TOOLS=[{
 ...
 name:'read_document',description:'Read text from a PDF imported or downloaded in this chat, or the saved content and typography of a generated artifact. Imported PDF text is read-only; it does not permit layout revisions. Omit versionId to read the selected version, or the latest version when none is selected.',
 ...
 return async call=>{
  ...
  if(call?.name==='read_document'){
   try{
    ...
    if(typeof v.content!=='string'&&v.format==='pdf'){
     const result=await pdfReference(artifacts.file(target),{signal});check();entryFor(entry.artifactId);
     return {artifactId:entry.artifactId,versionId:v.id,versionNumber:v.number,title:v.name,format:v.format,content:result.text,pages:result.pages,truncated:result.truncated,editable:false};
    }
