...
function validInteractions(value){return value===undefined||Array.isArray(value)&&value.length<=20&&new Set(value.map(x=>x?.id)).size===value.length&&value.every(x=>x&&isText(x.id,256)&&['clarification','approval'].includes(x.kind)&&isText(x.question,1500)&&['waiting','answered','denied','cancelled'].includes(x.status)&&Number.isFinite(x.createdAt)&&x.createdAt>=0&&(x.options===undefined||Array.isArray(x.options)&&x.options.length<=4&&x.options.every(o=>isText(o,200)))&&(x.answer===undefined||isText(x.answer,8000))&&(x.tool===undefined||isText(x.tool,128))&&(x.approvalAction===undefined||['send_email','create_event','reschedule_event','cancel_event','upload_file'].includes(x.approvalAction))&&(x.onceOnly===undefined||typeof x.onceOnly==='boolean')&&(x.detail===undefined||isText(x.detail,x.onceOnly?64000:2000)));}
...
class ChatInteractions{
 ...
 respond({conversationId,id,answer,decision}={}){
  ...
  if(entry.card.onceOnly&&decision==='chat')throw Error('This action must be approved once for this message.');
 ...
 async approve(conversationId,run,tool,detail,{required=false,question,onceOnly=false,approvalAction}={}){this.assertRun(conversationId,run);if(!onceOnly&&(!required&&this.host.conversation(conversationId).approvalMode!=='ask'||this.grants.get(conversationId)?.has(tool)))return true;const result=await this.request(conversationId,run,{kind:'approval',tool,...(onceOnly?{onceOnly:true}:{}),...(approvalAction?{approvalAction}:{}),question:question??`Allow ${tool.replace(/^hosted:/,'').replaceAll('_',' ')}?`,...(detail?{detail:onceOnly?detail:detail.slice(0,2000)}:{})});this.assertRun(conversationId,run);return result.allowed;}
