...
class ChatInteractions{
 ...
 async approve(conversationId,run,tool,detail){this.assertRun(conversationId,run);if(this.host.conversation(conversationId).approvalMode!=='ask'||this.grants.get(conversationId)?.has(tool))return true;const result=await this.request(conversationId,run,{kind:'approval',tool,question:`Allow ${tool.replace(/^hosted:/,'').replaceAll('_',' ')}?`,...(detail?{detail:detail.slice(0,2000)}:{})});this.assertRun(conversationId,run);return result.allowed;}
