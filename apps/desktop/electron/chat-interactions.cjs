...
class ChatInteractions{
 ...
 async approve(conversationId,run,tool,detail,{required=false,question,onceOnly=false}={}){this.assertRun(conversationId,run);if(!onceOnly&&(!required&&this.host.conversation(conversationId).approvalMode!=='ask'||this.grants.get(conversationId)?.has(tool)))return true;const result=await this.request(conversationId,run,{kind:'approval',tool,...(onceOnly?{onceOnly:true}:{}),question:question??`Allow ${tool.replace(/^hosted:/,'').replaceAll('_',' ')}?`,...(detail?{detail:onceOnly?detail:detail.slice(0,2000)}:{})});this.assertRun(conversationId,run);return result.allowed;}
