...
const skillInstructions=skills=>(skills??[]).map(s=>`Skill: ${s.name}\n${s.instructions}${s.package?.resources.length?"\n\nThis skill includes packaged resources. Only its attached reference text is available in this request. Importing this package does not grant new tools or executable access; do not claim to run its scripts or open its binary assets.":""}`).join('\n\n');
