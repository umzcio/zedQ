...
import { freshWorkspace, unwrap, type WorkspaceState, type FileDocument } from '@zq/module-api'
...
export default function DesktopRoot({modules}:{modules:LoadedModule[]}){
 ...
 useEffect(()=>{
  ...
  Promise.all([unwrap(window.zq.workspace.load()),unwrap(window.zq.files.list())]).then(([workspace,files])=>{
   if(!mounted)return
   const state=workspace??freshWorkspace(); latest.current=state;fileDrafts.current=files;setLoaded({workspace:state,files})
 ...
 const save=useCallback((snapshot:WorkspaceState)=>{
  ...
  setStatus('Saving…')
  pending.current=pending.current.catch(()=>{}).then(()=>unwrap(window.zq.workspace.save(snapshot)))
  pending.current.then(()=>{if(sequence===revision.current)setStatus('Saved on this Mac')},()=>{if(sequence===revision.current)setStatus('Unable to save — keep zQ open')})
 },[])
 const flushWorkspace=useCallback(async()=>{await pending.current.catch(()=>{});if(!latest.current)throw Error('Workspace is not ready.');await unwrap(window.zq.workspace.save(latest.current))},[])
