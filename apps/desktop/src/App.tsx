...
export default function App({draftIssues=[],initialState,initialFiles,onSnapshot,onFilesChange,saveStatus,onWorkspaceFlush,onFileFlush,closing,modules}:Props){
 ...
 const services=useMemo(()=>({code:window.zq.code,connectors:window.zq.connectors,clipboard:window.zq.clipboard,artifacts:window.zq.artifacts,voice:window.zq.voice,files:window.zq.files,chat:window.zq.chat,attachments:window.zq.attachments}),[])
 function servicesFor(manifest:ModuleManifest){return Object.fromEntries(Object.entries(services).map(([key,value])=>[key,manifest.capabilities.includes(key==='code'?'code.v1':key)?value:new Proxy({},{get(){throw Error(`Module ${manifest.id} cannot access ${key}`)}})])) as typeof services}
