...
export default function App({initialState,initialFiles,onSnapshot,onFilesChange,saveStatus,onWorkspaceFlush,onFileFlush,closing,modules}:Props){
 ...
 const targets=useMemo(()=>({view:viewTarget,sidebar:sidebarTarget,settings:settingsTarget,skillsSettings:skillsSettingsTarget}),[viewTarget,sidebarTarget,settingsTarget,skillsSettingsTarget])
 ...
 const services=useMemo(()=>({clipboard:window.zq.clipboard,artifacts:window.zq.artifacts,voice:window.zq.voice,files:window.zq.files,chat:window.zq.chat,attachments:window.zq.attachments}),[])
 ...
 return <SelectInteractionContext.Provider value={closing}><div className={`app-shell ${view==='Chat'?'chat-shell':''} ${!layout.sidebar||focusMode?'sidebar-hidden':''}`}>
  ...
  <div className="main-shell"><main className={`main-content view-${view.toLowerCase()}`}><div className="module-view-slot" ref={setViewTarget}/>{view==='Settings'&&<Settings section={settingsSection} theme={theme} palette={palette} setTheme={setTheme} setPalette={setPalette} closing={closing} settingsRef={setSettingsTarget} skillsSettingsRef={setSkillsSettingsTarget}/>}</main><footer className="app-status"><span><span className={`storage-dot ${saveStatus.startsWith('Unable')?'error':''}`}/>{saveStatus}</span></footer></div>
