...
import { IconContext, Check, House, NotePencil, Kanban, MagnifyingGlass, GearSix, ChatCircle, TerminalWindow, Sparkle, Sun, Moon, Desktop, Plus, SidebarSimple, Package, ArrowClockwise, Microphone, Scroll } from '@phosphor-icons/react'
...
export default function Navigation(p:Props){
 ...
 return <IconContext.Provider value={{weight:'light'}}>
  ...
  <aside id="context-navigation" className="context-sidebar" aria-label={`${p.view} navigation`} hidden={p.collapsed}>
   ...
   <div className="context-scroll"><div className="module-sidebar-slot" ref={p.sidebarRef}/>{p.view==='Settings'&&<nav className="context-links settings-navigation" aria-label="Settings sections">{([{id:'general',label:'General',icon:GearSix},{id:'appearance',label:'Appearance',icon:Sun},{id:'connections',label:'AI providers',icon:ChatCircle},{id:'connectors',label:'Connectors',icon:ConnectorIcon},{id:'skills',label:'Skills',icon:Scroll},{id:'voice',label:'Voice',icon:Microphone},{id:'modules',label:'Modules',icon:Package},{id:'updates',label:'Updates',icon:ArrowClockwise}] as const).map(({id,label,icon:Icon})=><TooltipButton key={id} tooltip={({'general':false,'appearance':false,'connectors':false,'connections':'Manage AI providers and choose available models','skills':false,'voice':false,'modules':'Manage installed modules and stage signed updates','updates':false})[id]} disabled={p.disabled} className={p.settingsSection===id?'selected':''} aria-current={p.settingsSection===id?'page':undefined} onClick={()=>p.onSettingsSection(id)}><Icon size={18}/><span>{label}</span></TooltipButton>)}</nav>}</div>
  </aside>
