 ...
 return <IconContext.Provider value={{weight:'light'}}>
  <header className="workspace-header" inert={p.disabled}>
   <div className="header-start">
    ...
    <nav className="header-modules" aria-label="Modules">{modules.map(({name,icon:Icon})=><TooltipButton key={name} tooltip={`Open ${name}`} aria-label={`${name} module`} aria-current={p.view===name?'page':undefined} className={p.view===name?'selected':''} onClick={()=>p.navigate(name)}><Icon size={18}/><span>{name}</span></TooltipButton>)}</nav>
   </div>
