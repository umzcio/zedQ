...
Keychain requests are serialized within the desktop credential store. Automatic connector restore, later OAuth token refresh, and retired-provider-key cleanup disable Keychain interaction in the native helper process. If a saved connector needs authorization, it stops with a reconnect action instead of showing a password dialog in the background. Explicit Connect and provider actions can still request approval; each interactive request allows two minutes, excluding its time in the queue. Cancellation releases the queue. Development ad-hoc signing still cannot promise durable trust across changed helper binaries. Moving from an ad-hoc development build to the Developer ID release can require authorizing existing items again. Both builds use the same local Application Support workspace; installing the app does not import personal data from the DMG.

Regression checks cover silent restore failure and explicit reconnect, noninteractive token refresh, and native denial using an independently signed helper against a disposable test credential. No normal-workspace credentials are accessed by these tests.
