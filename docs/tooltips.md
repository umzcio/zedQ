...
Pointer delay is 450 ms, with a 300 ms window for immediate adjacent tooltips. Keyboard focus is immediate; Escape dismisses help. Tooltips use neutral theme surfaces, a 300px maximum width, collision padding and restrained 125ms entry motion. Subsequent tooltips and reduced-motion preference skip that animation. Content remains hoverable and contains no interactive controls. Native disabled controls remain disabled, with a focusable help wrapper; module CSS preserves affected tab/select layouts.
