...
export function ProjectDialog({ project, remove=false, renameOnly=false, closing, chat, onClose }: {project?:ChatProject;remove?:boolean;renameOnly?:boolean;closing:boolean;chat:ChatController;onClose:()=>void}) {
 ...
 async function save(){
  ...
  try{if(remove&&project)await chat.deleteProject(project.id);else{const saved=await chat.saveProject({id:savedProjectId.current,name,...(renameOnly?{}:{instructions})});savedProjectId.current=saved.id;if(!renameOnly)await chat.updateProject({id:saved.id,defaultModel,defaultTools:defaultTools.filter(tool=>options.some(option=>option.kind===tool))})}onClose()}
 ...
 return <Dialog open onOpenChange={open=>{if(!open&&!blocked)onClose()}}><DialogContent className="project-dialog" showCloseButton={!blocked} onOpenAutoFocus={e=>{e.preventDefault();if(remove)cancel.current?.focus();else{input.current?.focus();input.current?.select()}}}>
  ...
  <form onSubmit={e=>{e.preventDefault();void save()}}>
   {!remove&&<><label htmlFor="project-name">Name</label><Input ref={input} id="project-name" aria-label="Project name" maxLength={256} value={name} disabled={blocked} onChange={e=>setName(e.target.value)} placeholder="What are you working on?"/>
    {!renameOnly&&<><label htmlFor="project-instructions">Instructions</label><Textarea id="project-instructions" aria-label="Project instructions" maxLength={16000} value={instructions} disabled={blocked} onChange={e=>setInstructions(e.target.value)} placeholder="Tell zQ what this project is about and how you’d like it to respond."/><div className="project-default-controls"><label>Default model</label><SelectField label="Project default model" value={defaultModel?JSON.stringify(defaultModel):''} onValueChange={value=>{setDefaultModel(value?JSON.parse(value):null);setDefaultTools([])}} disabled={blocked} options={[{value:'',label:'Use workspace default'},...modelOptions]}/>{options.length>0&&<div><label>Default tools</label>{options.map(option=><label key={option.kind} className="chat-tool-option"><Checkbox disabled={blocked} checked={defaultTools.includes(option.kind)} onCheckedChange={checked=>setDefaultTools(old=>checked===true?[...old,option.kind]:old.filter(tool=>tool!==option.kind))}/><span>{option.label}</span></label>)}</div>}<p>Defaults apply to new conversations. You can choose another model or tools for an individual chat.</p>{toolError&&<p role="alert">{toolError}</p>}</div></>}
   </>}
   {error&&<p className="chat-action-error" role="alert">{error}</p>}
   <div className="dialog-actions"><Button ref={cancel} type="button" variant="ghost" disabled={blocked} onClick={onClose}>Cancel</Button><Button type="submit" variant={remove?'destructive':'default'} className={remove?'chat-delete-confirm':'primary-button'} disabled={blocked||!remove&&(!name.trim()||!renameOnly&&(!ready||!!toolError))}>{busy?'Saving…':remove?'Delete project':renameOnly?'Rename':project?'Save changes':'Create project'}</Button></div>
  </form>
