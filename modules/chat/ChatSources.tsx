...
export default function ChatSources({message,closing,onOpen}:{message:ChatMessage;closing:boolean;onOpen:()=>void}){
 ...
 return <button type="button" className="chat-sources-pill" disabled={closing} onClick={onOpen} aria-label={`View ${sources.length} ${linked?'linked sources':'sources'}`}><span className="chat-source-badges" aria-hidden="true">{domains.map(host=><SourceIcon key={host} host={host}/>)}</span><span>{sources.length} {linked?'linked sources':sources.length===1?'source':'sources'}</span></button>
}
...
function SearchActivity({tool}:{tool:ChatToolActivity}){
 ...
 const row=<div className="source-activity-row"><Globe size={16}/><Collapsible className="source-activity-detail"><CollapsibleTrigger disabled={!tool.detail} className="source-activity-trigger"><span>{label}</span>{tool.detail&&<><span className="source-query" title={tool.detail}>{tool.detail}</span><CaretRight size={12}/></>}</CollapsibleTrigger>{tool.detail&&<CollapsibleContent><pre>{tool.detail}</pre></CollapsibleContent>}</Collapsible>{tool.detail&&<button className="chat-source-actions" aria-label="Copy search details" disabled={closing} onClick={()=>void copy()}><Copy size={15}/></button>}</div>
...
export function SourcesPane({chat,message}:{chat:ChatController;message:ChatMessage}){
 ...
 return <aside className="artifact-pane sources-pane" aria-label="Sources pane"><header className="artifact-pane-header"><h2>Sources</h2><Button variant="ghost" size="icon-sm" data-pane-close aria-label="Close sources" onClick={()=>chat.closeArtifact()}><X size={18}/></Button></header><div className="sources-pane-scroll" tabIndex={0} role="region" aria-label="Sources and search activity">
  {message.thinking&&<Collapsible className="source-thinking"><CollapsibleTrigger><Lightbulb size={17}/><span>Thinking</span><CaretRight size={13}/></CollapsibleTrigger><CollapsibleContent><pre>{message.thinking}</pre></CollapsibleContent></Collapsible>}
  ...
  <ol className="chat-sources-list">{sources.map(source=><ContextMenu key={source.url}><ContextMenuTrigger asChild><li data-source-url={source.url} data-selected={chat.sourceTarget?.sourceUrl===source.url||undefined}><a aria-current={chat.sourceTarget?.sourceUrl===source.url?true:undefined} href={source.url} title={source.title} className="chat-source-link" aria-disabled={closing} onClick={e=>{e.preventDefault();void open(source)}}><SourceIcon host={domain(source)}/><span className="chat-source-number">{source.id}</span><SourceDescription source={source}/><ArrowUpRight size={14}/></a><DropdownMenu><DropdownMenuTrigger asChild><button type="button" className="chat-source-actions" disabled={closing} aria-label={`Actions for source ${source.title}`}><DotsThree size={17}/></button></DropdownMenuTrigger><DropdownMenuContent align="end">{actions(source).map(a=><DropdownMenuItem key={a.label} disabled={closing} onSelect={a.run}>{a.label}</DropdownMenuItem>)}</DropdownMenuContent></DropdownMenu></li></ContextMenuTrigger><ContextMenuContent>{actions(source).map(a=><ContextMenuItem key={a.label} disabled={closing} onSelect={a.run}>{a.label}</ContextMenuItem>)}</ContextMenuContent></ContextMenu>)}</ol>
 </div></aside>
}
