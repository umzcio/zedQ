...
import { TooltipButton, Button, Dialog, DialogContent, DialogTitle, DialogDescription, ContextMenu, ContextMenuTrigger, ContextMenuContent, ContextMenuItem, DropdownMenu, DropdownMenuTrigger, DropdownMenuContent, DropdownMenuItem } from '@zq/ui'
...
export const SkillPackageFiles = forwardRef<SkillPackageFilesHandle, Props>(function SkillPackageFiles({ resources, frontmatter, activePaths, skillId, disabled, onBusyChange, onPreviewChange }, ref) {
 ...
 return <section className="skill-package-files" aria-label="Skill package files">
  {resources.length > 0 && <><div className="skill-files-heading"><label>Package files <span className="skill-optional">{resources.length}</span></label></div><p className="skill-help">Original paths and file contents are preserved for export. Only active references are included in chat context. Scripts are never run.</p>
   <div className="skill-files">{resources.map(resource => <ContextMenu key={resource.path}><ContextMenuTrigger asChild disabled={blocked}><div className="skill-file skill-package-file"><TooltipButton ref={node => { if (node) rows.current.set(resource.path, node); else rows.current.delete(resource.path) }} type="button" className="skill-file-name" disabled={blocked} aria-label={`Preview package file ${resource.path}`} tooltip={`Preview ${resource.path}. ${activePaths.includes(resource.path) ? 'Included as reference context when this skill is used.' : 'Preserved for export; not included in chat context.'}`} onClick={() => void openFile(resource)}><File size={16}/><span><strong>{resource.path}</strong><small>{activePaths.includes(resource.path) ? 'Active context' : 'Preserved for export'} · {resource.size < 1024 ? `${resource.size} B` : `${(resource.size / 1024).toFixed(1)} KB`}</small></span></TooltipButton><DropdownMenu><DropdownMenuTrigger asChild><Button type="button" variant="ghost" size="icon" disabled={blocked} aria-label={`Actions for package file ${resource.path}`}><DotsThree size={17}/></Button></DropdownMenuTrigger><DropdownMenuContent align="end" onCloseAutoFocus={closeMenu}><DropdownMenuItem disabled={blocked} onSelect={() => { opening.current = true; void openFile(resource) }}><Eye size={15}/>Preview</DropdownMenuItem>{skillId && <DropdownMenuItem disabled={blocked} onSelect={() => void saveFile(resource)}><DownloadSimple size={15}/>Save file</DropdownMenuItem>}</DropdownMenuContent></DropdownMenu></div></ContextMenuTrigger><ContextMenuContent onCloseAutoFocus={closeMenu} aria-label={`Package file actions for ${resource.path}`}><ContextMenuItem disabled={blocked} onSelect={() => { opening.current = true; void openFile(resource) }}><Eye size={15}/>Preview</ContextMenuItem>{skillId && <ContextMenuItem disabled={blocked} onSelect={() => void saveFile(resource)}><DownloadSimple size={15}/>Save file</ContextMenuItem>}</ContextMenuContent></ContextMenu>)}</div>
  </>}
  {frontmatter && <details className="skill-package-metadata"><ControlTooltip content="View the preserved package metadata and compatibility requirements; import does not grant permissions"><summary>Package metadata</summary></ControlTooltip><p className="skill-help">Original metadata is retained for export, including compatibility requirements. Importing it does not grant tools or permissions.</p><pre className="skill-import-text">{frontmatter}</pre></details>}
