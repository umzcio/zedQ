...
import { Button, Input } from '@zq/ui'
...
export default function SkillsSettings({ chat, closing }: { chat: ChatController; closing: boolean }) {
 ...
 return <section className="skills-settings" id="chat-skills">
  <div className="settings-panel-header"><div><h2>Skills</h2><p>Instructions and references you can reuse across chats and projects.</p></div><div className="skill-library-actions"><Button ref={browseButton} type="button" variant="ghost" disabled={disabled||actions.busy} onClick={()=>setBrowsing(true)}>Browse skills</Button><SkillImportControl disabled={disabled||actions.busy||skills.length>=100}/><Button variant="outline" disabled={disabled || actions.busy || skills.length >= 100} onClick={actions.create}><Plus size={15}/>New skill</Button></div></div>
