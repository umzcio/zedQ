...
export function ChatToolPicker({options,selected,onChange,disabled}:{options:ChatToolOption[];selected:ChatToolKind[];onChange:(tools:ChatToolKind[])=>void;disabled:boolean}){
 ...
 return <Popover open={open&&!disabled} onOpenChange={setOpen}><PopoverTrigger asChild><button type="button" className={`chat-tools-trigger ${selected.length?'has-tools':''}`} disabled={disabled} aria-label="Choose provider tools"><SlidersHorizontal size={16}/><span>Tools{selected.length?` · ${selected.length}`:''}</span></button></PopoverTrigger><PopoverContent side="top" align="start" className="chat-tools-menu" aria-label="Provider tools"><div className="chat-tools-heading">Tools for this model</div>{options.map(option=><label key={option.kind} className="chat-tool-option"><Checkbox checked={selected.includes(option.kind)} onCheckedChange={checked=>onChange(checked===true?[...selected,option.kind]:selected.filter(kind=>kind!==option.kind))}/><span>{option.label}<small>{option.description}</small></span></label>)}<p>Provider usage charges may apply.</p></PopoverContent></Popover>
}
...
function SearchSummary({summary,closing,onOpen}:{summary:NonNullable<ReturnType<typeof searchActivitySummary>>;closing:boolean;onOpen:()=>void}){
 return <ContextMenu><ContextMenuTrigger asChild><button type="button" disabled={closing} className="chat-tool-summary chat-search-summary" onClick={onOpen} aria-label={`${summary.label}, ${summary.count} ${summary.count===1?'search':'searches'}. View search activity`}><span className={summary.running?'chat-tool-running':''}><Globe size={15}/></span><span>{summary.label}{summary.running?'…':''}</span>{summary.count>1&&<span className="chat-search-count">{summary.count} searches</span>}<CaretRight size={12}/></button></ContextMenuTrigger><ContextMenuContent><ContextMenuItem disabled={closing} onSelect={onOpen}>View search activity</ContextMenuItem></ContextMenuContent></ContextMenu>
}
