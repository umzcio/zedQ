...
export default memo(function ChatCode({code,language}:{code:string;language:string}){
 ...
 return <ContextMenu><ContextMenuTrigger asChild><div className={`chat-code-block ${wrap?'is-wrapped':''}`}>
  <div className="chat-code-toolbar"><span>{language||'text'}</span><div><button type="button" title={wrap?'Scroll long lines':'Wrap long lines'} aria-label={wrap?'Scroll long code lines':'Wrap long code lines'} aria-pressed={wrap} onClick={()=>setWrap(!wrap)}><TextAlignLeft size={14}/></button><button type="button" title="Save code as file" aria-label="Save code as file" disabled={saving||closing} onClick={()=>void save()}><DownloadSimple size={14}/></button><button type="button" title={copied?'Copied':'Copy code'} aria-label={copied?'Code copied':'Copy code'} onClick={()=>void copy()}>{copied?<Check size={14}/>:<Copy size={14}/>}</button></div></div>
