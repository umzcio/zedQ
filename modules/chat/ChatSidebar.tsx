import {ResearchMenu} from './ResearchCard'
import {researchActive,researchLabel} from './useResearch'
import {SidebarSection} from '@zq/ui'
import { TooltipButton } from '@zq/ui'
import { useEffect, useState } from 'react'
import { Plus, SquaresFour, MagnifyingGlass, FolderSimple, ChatsCircle, CaretRight, DotsThree, PushPin } from '@phosphor-icons/react'
...
export default function ChatSidebar({ chat, closing }: {
 ...
 function renderConversation(c:Conversation) {
  const match=matchFor(c),researchJob=chat.research.jobs.find(j=>j.conversationId===c.id&&researchActive(j))
  ...
  return <ChatContextMenu chat={chat} key={c.id} conversation={c} disabled={closing} deleteDisabled={!!researchJob || !!chat.fileBusy[c.id] || c.messages.some(m => m.status === 'streaming')} onOpen={open} onAction={setAction}><TooltipButton tooltip={`Open ${c.title}${match ? ' at the matching message' : ''}. Right-click for chat actions.`} className={`chat-recent ${!chat.artifactsView && !chat.projectHome && chat.conversation?.id === c.id ? 'selected' : ''}`} aria-current={!chat.artifactsView && !chat.projectHome && chat.conversation?.id === c.id ? 'page' : undefined} disabled={closing} onClick={open} onKeyDown={event => { if (event.key === 'F2') { event.preventDefault(); setAction({ kind: 'rename', conversation: c }) } }}>
   ...
   {researchJob&&<span className="chat-research-indicator" aria-label={researchLabel(researchJob)}>{researchLabel(researchJob)}</span>}
 ...
 return <div className="chat-navigation">
  ...
  {chat.research.jobs.some(job=>researchActive(job)||['failed','interrupted'].includes(job.status))&&<SidebarSection storageKey="chat.research" title="Research"><div className="research-sidebar-list">{chat.research.jobs.filter(job=>researchActive(job)||['failed','interrupted'].includes(job.status)).map(job=><ResearchMenu key={job.id} research={chat.research} job={job}><button type="button" className="research-sidebar-row" disabled={closing} onClick={()=>chat.research.open(job)}><MagnifyingGlass size={16}/><span><strong>{job.title}</strong><small>{researchLabel(job)}</small></span></button></ResearchMenu>)}</div></SidebarSection>}
  <SidebarSection storageKey="chat.projects" title="Projects" className="chat-projects" actions={<TooltipButton className="header-icon" aria-label="New project" tooltip="Create a project with shared instructions, files, and chats" disabled={closing||chat.loading} onClick={()=>setNewProject(true)}><Plus size={15}/></TooltipButton>}>
  ...
  <SidebarSection storageKey="chat.recents" title={scope==='active'?'Recents':scope==='archived'?'Archive':'Trash'} className="chat-recents" actions={<DropdownMenu><DropdownMenuTrigger asChild><TooltipButton type="button" aria-label="Conversation scope" tooltip="Choose active chats, archived chats, or Trash"><DotsThree size={15}/></TooltipButton></DropdownMenuTrigger><DropdownMenuContent align="end">{([{value:'active',label:'Active chats'},{value:'archived',label:'Archive'},{value:'trash',label:'Trash'}] as const).map(item=><DropdownMenuItem key={item.value} onSelect={()=>setScope(item.value)}>{item.label}{scope===item.value?' ✓':''}</DropdownMenuItem>)}</DropdownMenuContent></DropdownMenu>}>
