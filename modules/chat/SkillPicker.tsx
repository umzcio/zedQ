...
export default function SkillPicker({ skills, selected, inherited, inheritedConfigured = inherited.length > 0, onChange, disabled, allowInherit = true, onManage }: SkillPickerProps) {
 ...
 return <><Popover open={open} onOpenChange={value => { if (!blocked) { setOpen(value); if (!value) setQuery('') } }}><PopoverTrigger asChild><Button tooltip="Choose skills to guide responses with reusable instructions and references" ref={trigger} type="button" variant="ghost" className="skill-picker-trigger" disabled={blocked} aria-label={`Skills${active.length ? `, ${active.length} enabled` : ''}`}><Scroll size={15}/><span>Skills</span>{active.length > 0 && <span className="skill-count">{active.length}</span>}<CaretDown size={12}/></Button></PopoverTrigger><PopoverContent className="skill-picker" align="start" onCloseAutoFocus={event => { if (opening.current) { event.preventDefault(); opening.current = false } }}>
  <div className="skill-picker-heading"><strong>Skills</strong><span>{active.length}/10 enabled</span></div>
  {allowInherit && <label className="skill-inherit"><Checkbox tooltip={selected === null ? 'Switch to a manual skill selection' : inheritedConfigured ? 'Use the skills selected by this project' : 'Let zQ choose relevant installed skills for each message'} aria-label={defaultLabel} checked={selected === null} disabled={blocked} onCheckedChange={checked => onChange(checked === true ? null : [...active])}/><span>{defaultLabel}<small>{defaultDescription}</small></span></label>}
  ...
  <div className="skill-picker-footer"><Button type="button" variant="ghost" disabled={blocked || selected !== null && active.length === 0} tooltip="Turn off automatic, project, and manually selected skills for this selection" onClick={() => onChange([])}>No skills</Button><Button type="button" variant="ghost" disabled={blocked} tooltip="Open the skill library to create, import, or edit skills" onClick={() => { setOpen(false); requestSkillSettingsPage('library'); onManage() }}><GearSix size={14}/>Manage skills</Button></div>
