...
export default function ComposerAddMenu({ research, connectorProps, skills, selected, inherited, inheritedConfigured = inherited.length > 0, disabled, skillDisabled, open, onOpenChange, onChange, onUpload, onNotes, onContext, onManage, onBrowse, onSkillAction, approvalMode, approvalDisabled, onApprovalMode }: Props) {
 ...
 if(research)return <DropdownMenu open={open&&!disabled} onOpenChange={onOpenChange}><DropdownMenuTrigger asChild><TooltipButton type="button" className="chat-attach" aria-label={open?'Close attachments menu':'Add attachments'} tooltip="Upload research files" disabled={disabled}><Plus size={21}/></TooltipButton></DropdownMenuTrigger><DropdownMenuContent side="top" align="start" className="composer-add-menu" aria-label="Add research material" onCloseAutoFocus={event=>{if(opening.current){event.preventDefault();opening.current=false}}}><DropdownMenuItem onSelect={()=>choose(onUpload)}><UploadSimple size={17}/>Upload files</DropdownMenuItem></DropdownMenuContent></DropdownMenu>
 return <DropdownMenu open={open && !disabled} onOpenChange={value => { onOpenChange(value); if (!value) setSubOpen(false) }}><DropdownMenuTrigger asChild><TooltipButton type="button" className="chat-attach" aria-label={open ? 'Close attachments menu' : 'Add attachments'} tooltip="Add files or notes, choose skills and connectors, or inspect message context" disabled={disabled}><Plus size={21}/></TooltipButton></DropdownMenuTrigger><DropdownMenuContent side="top" align="start" className="composer-add-menu" aria-label="Add attachments and skills" onCloseAutoFocus={event => { if (opening.current) { event.preventDefault(); opening.current = false } }}>
  ...
  <ConnectorSubmenu {...connectorProps}/>
