...
import { ContextMenu, ContextMenuTrigger, ContextMenuContent, ContextMenuItem, ContextMenuSeparator, ContextMenuSub, ContextMenuSubTrigger, ContextMenuSubContent } from '@zq/ui'
...
export function ProjectIconPicker({project,chat,disabled=false}:{project:ChatProject;chat:ChatController;disabled?:boolean}){
 ...
 return <div className="project-icon-picker" onKeyDown={e=>{if(e.key!=='Escape'&&e.key!=='Tab')e.stopPropagation()}}>
  ...
  <div className="project-icon-grid" role="group" aria-label="Project icons" onKeyDown={e=>{
   ...
   {names.map(name=><button key={name} type="button" aria-label={`${name.replace(/([a-z])([A-Z])/g,'$1 $2')} icon`} title={name.replace(/([a-z])([A-Z])/g,'$1 $2')} aria-pressed={(project.icon??'FolderSimple')===name} disabled={disabled||busy} onClick={()=>void save({icon:name})}><ProjectIcon project={{icon:name,color:project.color}} size={23}/></button>)}
   {!names.length&&<p>No matching icons.</p>}
  </div>
  <div className="project-icon-colors" role="group" aria-label="Icon color">{projectColors.map(color=><button key={color.id} type="button" aria-label={`${color.label} icon color`} title={color.label} aria-pressed={(project.color??'neutral')===color.id} disabled={disabled||busy} style={{background:color.dark}} onClick={()=>void save({color:color.id})}>{(project.color??'neutral')===color.id&&<Check size={13} weight="bold"/>}</button>)}</div>
