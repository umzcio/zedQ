...
import { TooltipButton, Button, Input, Textarea, Dialog, DialogContent, DialogTitle, DialogDescription, ContextMenu, ContextMenuTrigger, ContextMenuContent, ContextMenuItem, ContextMenuSeparator, DropdownMenu, DropdownMenuTrigger, DropdownMenuContent, DropdownMenuItem, DropdownMenuSeparator } from '@zq/ui'
  ...
  <form onSubmit={event => { event.preventDefault(); void save() }}>
   {action !== 'delete' && <><label htmlFor={`${id}-name`}>Name</label><Input id={`${id}-name`} ref={input} aria-label="Skill name" value={name} maxLength={256} disabled={blocked} onChange={event => setName(event.target.value)} placeholder="e.g. Writing style"/>
    {action !== 'rename' && <><label htmlFor={`${id}-description`}>Description <span className="skill-optional">Optional</span></label><Input id={`${id}-description`} aria-label="Skill description" value={description} maxLength={1024} disabled={blocked} onChange={event => setDescription(event.target.value)} placeholder="When should this skill be used?"/>
     ...
     <div className="skill-files-heading"><label>Reference files <span className="skill-optional">{files.length}/10</span></label><Button type="button" variant="ghost" disabled={blocked || files.length >= 10} tooltip={files.length >= 10 ? 'Remove a reference before adding another; the limit is 10' : 'Attach reference files to include with this skill’s instructions'} onClick={() => void addFiles()}><Paperclip size={14}/>Add files</Button></div>
   ...
   <div className="dialog-actions"><Button type="button" ref={cancel} variant="ghost" disabled={blocked} onClick={close}>Cancel</Button><Button tooltip={action === 'delete' ? 'Delete this skill from the library; previously sent chats keep their snapshots' : !name.trim() ? 'Enter a skill name to save' : action === 'rename' ? 'Save the new skill name' : 'Save this skill’s name, description, and instructions'} type="submit" variant={action === 'delete' ? 'destructive' : 'default'} disabled={blocked || action !== 'delete' && !name.trim()}>{busy ? 'Saving…' : action === 'delete' ? 'Delete skill' : action === 'rename' ? 'Rename' : 'Save skill'}</Button></div>
  </form>
 </DialogContent></Dialog>
 <Dialog open={discarding} onOpenChange={setDiscarding}><DialogContent className="skill-discard-dialog"><DialogTitle>Discard skill changes?</DialogTitle><DialogDescription>Your unsaved name, description and instructions will be lost. Saved reference file changes are kept.</DialogDescription><div className="dialog-actions"><Button variant="ghost" onClick={() => setDiscarding(false)}>Keep editing</Button><Button variant="destructive" tooltip="Discard unsaved text edits; saved reference file changes are kept" onClick={onClose}>Discard changes</Button></div></DialogContent></Dialog>
