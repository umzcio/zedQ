...
export function ManageModels({connection,closing,onClose,onCloseAutoFocus}:{connection:Connection;closing:boolean;onClose:()=>void;onCloseAutoFocus:(event:Event)=>void}){
 ...
 return <><Dialog open onOpenChange={open=>{if(!open&&!saving&&!closing)onClose()}}><DialogContent className="model-manager-dialog" showCloseButton={!saving&&!closing} onCloseAutoFocus={onCloseAutoFocus} onEscapeKeyDown={e=>{if(saving||closing)e.preventDefault()}}>
  ...
  <div className="model-catalog-status"><ControlTooltip content={loading ? 'Requesting the latest model list from this provider' : catalog ? 'Models returned by the provider; select which ones appear in Chat' : 'Using saved model choices because a fresh provider list is unavailable'}><span role="status" tabIndex={0}>{loading?'Fetching available models…':catalog?`${catalog.length} available from this provider`:'Saved model selection'}</span></ControlTooltip><Button type="button" variant="ghost" size="icon" tooltip="Fetch the latest model list from this provider" aria-label="Refresh available models" disabled={loading||saving||closing} onClick={()=>setRefresh(v=>v+1)}><ArrowsClockwise size={16} className={loading?'model-refreshing':''}/></Button></div>
