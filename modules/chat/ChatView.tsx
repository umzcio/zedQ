...
export default function ChatView({chat,notes,closing,settings,prepareNotes}:{prepareNotes:()=>Promise<void>;chat:ChatController;notes:Note[];closing:boolean;settings:()=>void}){
 ...
 return <div className={`chat-view ${empty?'chat-is-empty':''}`} onKeyDown={e=>{if((e.metaKey||e.ctrlKey)&&e.key==='f'&&!empty){e.preventDefault();setFinding(true);findInput.current?.focus()}if(e.key==='Escape'&&finding){e.preventDefault();setFinding(false);input.current?.focus()}}} onDragEnter={e=>{if(e.dataTransfer.types.includes('Files')){e.preventDefault();dragDepth.current++;setDragging(true)}}} onDragOver={e=>{if(e.dataTransfer.types.includes('Files')){e.preventDefault();e.dataTransfer.dropEffect=closing||pending||importing?'none':'copy'}}} onDragLeave={e=>{e.preventDefault();if(--dragDepth.current<=0){dragDepth.current=0;setDragging(false)}}} onDrop={e=>{e.preventDefault();dragDepth.current=0;setDragging(false);if(!closing&&!pending&&!importing&&e.dataTransfer.files.length)void chat.addFiles(draftId,Array.from(e.dataTransfer.files))}}>
  {readOnly&&<div className="chat-restoration"><span>{c?.deletedAt?'This chat is in Trash.':'This chat is archived.'}</span><Button variant="ghost" size="sm" onClick={()=>void chat.updateConversation({id:c!.id,deleted:false,archived:false}).catch(e=>setError(e.message))}>Restore chat</Button></div>}
  {activeProject&&<button className="chat-project-link" onClick={()=>chat.openProject(activeProject.id)}><ProjectIcon project={activeProject} size={16}/>{activeProject.name}</button>}
  ...
  {!empty&&<div className="chat-reading-tools">{finding?<div className="chat-find"><MagnifyingGlass size={14}/><input ref={findInput} aria-label="Find in conversation" placeholder="Find in conversation…" value={findQuery} onChange={e=>{setFindQuery(e.target.value);setMatchIndex(0)}} onKeyDown={e=>{if(e.key==='Enter'){e.preventDefault();findNext(e.shiftKey?-1:1)}}}/><span role="status">{findQuery.trim()?matches.length?`${matchIndex%matches.length+1} / ${matches.length}`:'No matches':''}</span><button type="button" aria-label="Previous match" disabled={!matches.length} onClick={()=>findNext(-1)}><CaretUp size={14}/></button><button type="button" aria-label="Next match" disabled={!matches.length} onClick={()=>findNext(1)}><CaretDown size={14}/></button><button type="button" aria-label="Close find" onClick={()=>{setFinding(false);input.current?.focus()}}><X size={14}/></button></div>:<button type="button" className="chat-find-trigger" title="Find in conversation (⌘F)" aria-label="Find in conversation" onClick={()=>setFinding(true)}><MagnifyingGlass size={14}/></button>}</div>}
  {!empty&&<div className="chat-messages" ref={scroller} onScroll={scrollChanged}>
   <div className="chat-thread" ref={motion.thread}>{c!.messages.map(m=><ChatMessageActions key={m.id} message={m} chat={chat} choice={choice} disabled={busy||pending||closing||readOnly} notes={notes} settings={settings} matched={finding&&matches.includes(m.id)} currentMatch={finding&&currentMatch===m.id}>
    ...
    {m.role==='assistant'&&['stopped','interrupted','error'].includes(m.status)&&<div className="chat-response-status">{m.status==='stopped'?'Stopped':m.status==='interrupted'?'Interrupted':'Couldn’t finish'}</div>}
  ...
  <div className="chat-compose-region">
   {!empty&&!atLatest&&<div className="chat-jump-latest"><button type="button" onClick={latest}><ArrowDown size={13}/>Jump to latest</button></div>}
   ...
   <div className="chat-composer-area" ref={motion.composer}>
    ...
    <form className="chat-composer" onSubmit={e=>{e.preventDefault();void send()}}>
     ...
     <div className="composer-actions">
      ...
      {busy&&<button type="button" className="chat-stop" aria-label="Stop response" title="Stop response" disabled={closing||stopping} onClick={()=>void stop()}><Stop size={15} weight="fill"/></button>}
      <button className={`chat-send${queueMode?' chat-enqueue':''}`} type="submit" title={queueMode?'Queue message':'Send message'} disabled={slash.blockBareSlash||!!contextError||readOnly||pending||closing||importing||!toolsReady||!!toolError||(!draft.trim()&&!ids.length&&!files.length)||!connection||!choice} aria-label={queueMode?'Queue message':'Send message'}><span key={queueMode?'queue':'send'} className="chat-send-glyph animate-in fade-in-0 zoom-in-95">{queueMode?<ListPlus size={19}/>:<ArrowUp size={19} weight="bold"/>}</span></button>
     </div>
    </form>
    {!chat.state.connections.length&&<button className="chat-setup-link" onClick={settings}>Connect a model to start chatting</button>}
   </div>
  </div>
  ...
  <Dialog open={attaching&&!closing} onOpenChange={setAttaching}><DialogContent inert={closing} className="attach-dialog"><DialogTitle>Attach notes</DialogTitle><DialogDescription>Choose notes to include with your message. Up to 10 files and notes total, with 100 KB of text.</DialogDescription><div className="model-search"><MagnifyingGlass size={15}/><input aria-label="Search notes to attach" placeholder="Find a note…" value={noteQuery} onChange={e=>setNoteQuery(e.target.value)}/></div><div className="attach-note-list">{notes.filter(n=>n.title.toLowerCase().includes(noteQuery.toLowerCase())).map(n=><label key={n.id}><input type="checkbox" checked={ids.includes(n.id)} disabled={!ids.includes(n.id)&&ids.length+files.length>=10} onChange={e=>chat.setAttachments(draftId,e.target.checked?[...ids,n.id]:ids.filter(id=>id!==n.id))}/><span>{n.title||'Untitled'}</span></label>)}</div><Button className="primary-button" onClick={()=>setAttaching(false)}>Done</Button></DialogContent></Dialog>
 </div>
}
