...
import {ControlTooltip,ArtifactDocumentView,Button,Input,SelectField,Dialog,DialogContent,DialogTitle,DialogDescription,ContextMenu,ContextMenuTrigger,ContextMenuContent,ContextMenuItem,DropdownMenu,DropdownMenuTrigger,DropdownMenuContent,DropdownMenuItem} from '@zq/ui'
...
import ResearchReport,{ResearchReportEditor,researchReportActions} from './ResearchReport'
...
function ArtifactPane({chat,expanded,canSplit,toggleExpanded}:{chat:ChatController;expanded:boolean;canSplit:boolean;toggleExpanded:()=>void}){
 ...
 const [reportEdit,setReportEdit]=useState<Artifact|null>(null)
 ...
 useEffect(()=>{let live=true;setLoaded(null);setError('');setZoom('fit');setRename(false);setRemove(false);setEdit(null);setReportEdit(null);setAttach(null);unwrap(services.artifacts.document(target)).then(file=>{if(live)setLoaded({key,file})}).catch(e=>{if(live)setLoaded({key,error:e.message})});return()=>{live=false}},[key])
 ...
 const actions=artifact&&version?[
  ...
  ...researchReportActions(artifact,target,chat,run,()=>setReportEdit(artifact),services),
 ...
 return <aside className="artifact-pane" aria-label="Artifact preview">
  <ContextMenu><ContextMenuTrigger asChild><header className="artifact-pane-header"><File size={17}/><div className="artifact-pane-title"><strong title={filename}>{filename}</strong><span>{format.toUpperCase()}{artifact?.deletedAt?' · Deleted':''}</span></div><Button variant="ghost" size="icon-sm" aria-label="Download artifact" tooltip="Save this artifact version to your computer" disabled={busy||closing||!version} onClick={()=>void run(async()=>{await unwrap(services.artifacts.save(target))})}><DownloadSimple size={17}/></Button><DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon-sm" aria-label="Artifact actions" tooltip="Rename, revise, attach, download, or delete this artifact" disabled={busy||closing||!version}><DotsThree size={18}/></Button></DropdownMenuTrigger><DropdownMenuContent align="end">{actions.map(a=><DropdownMenuItem key={a.label} disabled={busy||closing||a.disabled} onSelect={a.action}>{a.label}</DropdownMenuItem>)}</DropdownMenuContent></DropdownMenu>{canSplit&&<Button variant="ghost" size="icon-sm" aria-label={expanded?'Restore split view':'Expand artifact'} tooltip={expanded ? 'Show the chat and artifact side by side' : 'Use the full workspace to preview this artifact'} onClick={toggleExpanded}>{expanded?<ArrowsInSimple size={17}/>:<ArrowsOutSimple size={17}/>}</Button>}<Button variant="ghost" size="icon-sm" data-pane-close aria-label="Close artifact" tooltip="Close the preview and return to the chat (Escape)" onClick={()=>chat.closeArtifact()}><X size={17}/></Button></header></ContextMenuTrigger><ContextMenuContent>{actions.map(a=><ContextMenuItem key={a.label} disabled={busy||closing||a.disabled} onSelect={a.action}>{a.label}</ContextMenuItem>)}</ContextMenuContent></ContextMenu>
  <div className="artifact-pane-toolbar">{artifact&&version?<SelectField label="Artifact version" tooltip="Preview a saved version of this artifact" value={version.id} onValueChange={id=>chat.openArtifact({artifactId:artifact.id,versionId:id},false,{focus:false})} options={artifact.versions.slice().reverse().map(v=>({value:v.id,label:`Version ${v.number}`}))}/>:<span/>}{format!=='research'&&<SelectField label="Artifact zoom" tooltip="Set the document magnification or fit it to the pane width" value={String(zoom)} onValueChange={value=>setZoom(value==='fit'?'fit':Number(value))} options={[{value:'fit',label:'Fit width'},...['50','75','100','125','150','200'].map(value=>({value,label:`${value}%`}))]}/>}</div>
  {error&&<p className="artifact-pane-error" role="alert">{error}</p>}
  <div className="artifact-pane-document">{loaded?.key!==key?<div className="artifact-pane-status" role="status">Preparing document…</div>:loaded.error?<div className="artifact-pane-status" role="alert"><p>{loaded.error}</p><Button variant="ghost" onClick={()=>void run(async()=>{await unwrap(services.artifacts.save(target))})}>Download file</Button></div>:loaded.file?.format==='research'?<ResearchReport key={key} target={target}/>:loaded.file?<ArtifactDocumentView file={loaded.file} zoom={zoom} onEscape={close}/>:null}</div>
  ...
  <Dialog open={remove} onOpenChange={setRemove}><DialogContent><DialogTitle>Delete {filename}?</DialogTitle><DialogDescription>The artifact moves to Deleted. Existing attachments keep their saved versions.</DialogDescription>{error&&<p role="alert">{error}</p>}<div className="dialog-actions"><Button variant="ghost" onClick={()=>setRemove(false)}>Cancel</Button><Button tooltip="Move this artifact to Deleted; existing attachments keep their saved versions" disabled={busy} onClick={()=>artifact&&void run(async()=>{await unwrap(services.artifacts.update({id:artifact.id,deleted:true}));setRemove(false)})}>Delete artifact</Button></div></DialogContent></Dialog>
  {reportEdit&&<ResearchReportEditor artifact={reportEdit} target={target} onClose={()=>setReportEdit(null)} onSaved={a=>{setReportEdit(null);chat.openArtifact({artifactId:a.id,versionId:a.versions.at(-1)!.id})}}/>}
