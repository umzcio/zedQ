...
export function ModelChecklist({models,selected,onChange,disabled=false,available,labels,onRename,onMenuClose}:{models:string[];selected:string[];onChange:(models:string[])=>void;disabled?:boolean;available?:string[];labels?:Record<string,string>;onRename?:(model:string)=>void;onMenuClose?:(event:Event)=>void}){
 ...
 return <div className="model-checklist">
  ...
  <div className="model-checklist-toolbar"><span>{selected.length} selected</span><Button type="button" variant="ghost" disabled={disabled||!shown.length} tooltip={query ? 'Show every model matching this search in Chat' : 'Show all of this provider’s models in Chat'} onClick={()=>onChange([...new Set([...selected,...shown])])}>{query?'Select shown':'Select all'}</Button><Button type="button" variant="ghost" disabled={disabled||!selected.length} tooltip="Hide all models from this provider in the Chat selector" onClick={()=>onChange([])}>Clear all</Button></div>
  <div className="model-checklist-options" role="group" aria-label="Models to show in Chat">
   {shown.map(id=><ContextMenu key={id}><ContextMenuTrigger asChild disabled={disabled}><label className="model-checklist-row">
    <Checkbox tooltip={selected.includes(id) ? `Hide ${modelName(id,labels)} from the Chat model selector` : `Show ${modelName(id,labels)} in the Chat model selector`} aria-label={`Enable ${id}`} checked={selected.includes(id)} disabled={disabled} onCheckedChange={()=>toggle(id)}/>
    <span className="model-label"><strong>{modelName(id,labels)}</strong>{modelName(id,labels)!==id&&<small>{id}</small>}{available&&!available.includes(id)&&<ControlTooltip content="This saved model was not returned by the latest provider check. It may be unavailable even if you keep it selected."><small tabIndex={0}>Not in the latest provider list</small></ControlTooltip>}</span>
