...
import type { Attachment, ChatSnapshot, ChatToolKind } from '@zq/module-api'
export function useChat(){
 ...
 useEffect(()=>{if(!hydrated.current)return;for(const id of new Set([...Object.keys(drafts),...Object.keys(attachments),...Object.keys(files),...Object.keys(toolChoices),...Object.keys(skillChoices)])){const input={id,...(skillChoices[id]!==undefined?{skillIds:skillChoices[id]}:{}),text:drafts[id]??'',noteIds:attachments[id]??[],tools:toolChoices[id]??[],attachmentIds:(files[id]??[]).map(a=>a.id)},key=JSON.stringify(input);if(lastDrafts.current.get(id)!==key){lastDrafts.current.set(id,key);pendingDrafts.current.set(id,input)}}if(pendingDrafts.current.size)scheduleSave()},[drafts,attachments,files,toolChoices,skillChoices])
 ...
 async function select(id:string,messageId?:string,versionId?:string){invalidateArtifactRequests();setArtifactsView(false);const target=state.conversations.find(c=>c.id===id);if(!target){host.notify('This source chat is unavailable.');return}if(conversation?.id!==id){dismissSourcesForNavigation();dismissArtifactForNavigation()}if(messageId&&versionId&&!target.messages.some(m=>m.id===messageId&&m.activeVersionId===versionId)){try{await unwrap(services.chat.selectMessageVersion({conversationId:id,messageId,versionId}))}catch(e){host.notify((e as Error).message);return}}setProjectId(target.projectId??null);setSelected(id);setProjectHome(false);setJump(messageId??null)}
 ...
 useCommand('chat.open',({conversationId,messageId,versionId})=>{select(conversationId,messageId,versionId);host.navigate('Chat')})
