...
function NotesRoot(){
 ...
 useEffect(()=>{if(view==='Notes'&&selectedNote)setNotes(ns=>ns.map(n=>n.id===selectedNote?{...n,openedAt:Date.now()}:n))},[view,selectedNote,setNotes])
 ...
 function newNote(body=''){const id=crypto.randomUUID();setNotes(n=>[{id,title:'Untitled',body,project:'',updated:Date.now(),pinned:false},...n]);setTabState(s=>openTab(s,`note:${id}`));setView('Notes');setFocusMode(false)}
 ...
 function updateNote(patch:Partial<Note>){if(note)setNotes(ns=>ns.map(n=>n.id===note.id?{...n,...patch,updated:Date.now()}:n))}
 ...
 function noteEditor(){return note?<div className="editor-pane"><div className="editor-toolbar"><div className="breadcrumb note-breadcrumb"><FileText size={14}/><SelectField className="note-project-select" label="Note project" tooltip="Move this note to a project" value={note.project} disabled={closing} onValueChange={project=>updateNote({project})} options={[{value:'',label:'Personal notes'},...projects]}/><CaretRight size={12}/><span>Note</span></div><div className="toolbar-actions"><IconButton label={note.pinned?'Unpin note':'Pin note'} active={note.pinned} onClick={()=>updateNote({pinned:!note.pinned})}><PushPin size={17}/></IconButton><IconButton label="Create a task linked to this note" onClick={()=>{const selection=editor.current?.value.substring(editor.current.selectionStart,editor.current.selectionEnd);setTaskEdit({id:crypto.randomUUID(),title:selection?.slice(0,120)||note.title,description:'',project:note.project,status:'Inbox',priority:'Normal',noteId:note.id})}}><Kanban size={17}/></IconButton><IconButton label={focusMode?'Exit focus mode':'Focus on this note and hide the sidebar'} active={focusMode} onClick={()=>setFocusMode(f=>!f)}><SidebarSimple size={17}/></IconButton></div></div><div className="writing-surface"><div className="note-meta"><span className="mini-label">SCRATCHPAD</span></div><input className="note-title" aria-label="Note title" value={note.title} onChange={e=>updateNote({title:e.target.value})} placeholder="Untitled"/><ArtifactAttachments items={note.artifacts??[]} onChange={artifacts=>updateNote({artifacts})}/><textarea ref={editor} className="note-body" aria-label="Note content" value={note.body} onChange={e=>updateNote({body:e.target.value})} placeholder="Start anywhere. This space is yours." spellCheck={false}/><ChatSourceLinks text={note.body}/></div><footer className="editor-footer"><span>{note.body.trim()?note.body.trim().split(/\s+/).length:0} words</span><ControlTooltip content={`Note storage: ${saveStatus}`}><span tabIndex={0}>Plain text <span className="separator">·</span> {saveStatus} <Check size={12}/></span></ControlTooltip></footer></div>:<div className="empty-state"><p>A little space to think.</p><Button title="Create a new note (⌘N)" onClick={()=>newNote()}>New note</Button></div>}
 ...
 return <><ModuleSurface slot="sidebar"><>
     <div className="context-links local-file-open"><TooltipButton tooltip="Open a local text file (⌘O)" onClick={p.openFile}><FolderOpen size={17}/><span>Open file…</span><small>⌘O</small></TooltipButton></div>
     ...
     <nav className="context-links" aria-label="Note collections">{(['All notes','Pinned'] as const).map(label=><TooltipButton key={label} tooltip={false} className={p.noteFilter===label?'selected':''} onClick={()=>p.setNoteFilter(label)}>{label==='Pinned'?<PushPin size={17}/>:<NotePencil size={17}/>}<span>{label}</span><small>{p.notes.filter(n=>label==='All notes'||n.pinned).length}</small></TooltipButton>)}</nav>
     ...
     <SidebarSection storageKey="notes.scratchpads" title={p.noteFilter==='All notes'?'Your scratchpads':p.noteFilter} meta={filteredNotes.length}>
     <div className="context-note-list">{filteredNotes.map(n=><NoteContextMenu key={n.id} note={n} onOpen={p.openNote} onRename={p.renameNote} onTogglePin={p.toggleNotePin} disabled={p.disabled}><TooltipButton tooltip={`Open note: ${n.title||'Untitled'} · F2 to rename`} onKeyDown={e=>{if(e.key==='F2'){e.preventDefault();p.renameNote(n.id)}}} className={`note-list-item ${p.selectedNote===n.id?'selected':''}`} onClick={()=>p.openNote(n.id)}><div><FileText size={14}/><strong>{n.title||'Untitled'}</strong>{n.pinned&&<PushPin size={12}/>}</div></TooltipButton></NoteContextMenu>)}{!filteredNotes.length&&<p className="context-empty">No notes here yet.</p>}</div></SidebarSection></></ModuleSurface><ModuleSurface><div className={`notes-workspace ${focusMode?'focused':''}`}><div className="note-main"><div className="tabs"><WorkspaceTabs onRenameNote={renameNote} onToggleNotePin={toggleNotePin} state={tabState} onChange={setTabState} documents={[...notes.map(n=>({key:`note:${n.id}`,title:n.title||'Untitled',note:n})),...local.files.map(f=>({key:`file:${f.id}`,title:f.name,path:f.path,dirty:f.body!==f.savedBody}))]} onNew={()=>newNote()} disabled={closing}/><IconButton label={split?'Hide tasks split':'Show tasks split'} active={split} onClick={()=>setSplit(s=>!s)}><Columns size={18}/></IconButton></div><div className={`editor-and-board ${split?'is-split':''}`}>{activeFile?<LocalFileEditor busy={local.busy} file={activeFile} error={local.error} status={local.status} onEdit={body=>local.edit(activeFile.id,body)} onAction={kind=>void local.action(activeFile.id,kind)}/>:noteEditor()}{split&&<aside className="split-board"><header><span><Kanban size={17}/> Tasks alongside</span><IconButton label="Close tasks split" onClick={()=>setSplit(false)}><X size={16}/></IconButton></header><div className="split-task-list">{tasks.filter(t=>t.status!=='Done').map(t=><div className="split-task" key={t.id}><TooltipButton className="complete-circle" aria-label={`Complete ${t.title}`} onClick={()=>moveTask(t.id,'Done')}><Circle size={17}/></TooltipButton><TooltipButton tooltip={`Edit task: ${t.title}`} onClick={()=>setTaskEdit({...t})}><strong>{t.title}</strong><span>{t.project} · {t.status}</span></TooltipButton></div>)}<TooltipButton tooltip="Create a new task" className="quiet-add" onClick={()=>newTask()}><Plus size={15}/> Add task</TooltipButton></div></aside>}</div></div></div>{local.error&&!activeFile&&<div className="file-error" role="alert">{local.error}</div>}</ModuleSurface>
 {renameTarget&&<RenameNoteDialog key={renameTarget.id} note={renameTarget} disabled={closing} onClose={()=>setRenameTarget(null)} onSave={(id,title)=>{if(closing)return;setNotes(ns=>ns.map(n=>n.id===id?{...n,title,updated:Date.now()}:n));setRenameTarget(null);setNotice('Note renamed')}}/>}
</>
}
