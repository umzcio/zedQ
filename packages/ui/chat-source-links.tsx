...
export function ChatSourceLinks({text,onOpen}:{text:string;onOpen?:()=>void}){
 ...
 return <>{links.map((link,index)=>{let target;try{const url=new URL(link);target={conversationId:decodeURIComponent(url.pathname.slice(1)),messageId:url.searchParams.get('message')??undefined,versionId:url.searchParams.get('version')??undefined}}catch{return null}const open=()=>{if(commands.run('chat.open',target))onOpen?.()};return <ContextMenu key={link}><ContextMenuTrigger asChild><TooltipButton type="button" tooltip="Open the conversation this note came from" className="linked-note" disabled={closing} onClick={open}>Open source chat{links.length>1?` ${index+1}`:''}<ArrowUpRight size={14}/></TooltipButton></ContextMenuTrigger><ContextMenuContent><ContextMenuItem disabled={closing} onSelect={open}>Open source chat</ContextMenuItem></ContextMenuContent></ContextMenu>})}</>
}
