...
export function SelectField({label,value,onValueChange,options,className,disabled=false}:{label:string;value:string;onValueChange:(value:string)=>void;options:readonly Option[];className?:string;disabled?:boolean}){
 ...
 return <Select value={`value:${value}`} onValueChange={next=>{if(!blocked)onValueChange(next.slice(6))}} disabled={blocked} open={open&&!blocked} onOpenChange={setOpen}>
  <SelectTrigger size="sm" aria-label={label} className={cn('zq-select-trigger',className)}><SelectValue/></SelectTrigger>
