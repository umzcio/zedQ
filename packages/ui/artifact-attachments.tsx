...
export function ArtifactAttachments({items,onChange}:{items:ArtifactReference[];onChange:(items:ArtifactReference[])=>void}){
 ...
 return <>{!!items.length&&<div className="artifact-attachments" aria-label="Attached artifacts">{items.map(ref=>{const a=library.find(a=>a.id===ref.artifactId),v=a?.versions.find(v=>v.id===ref.versionId);return <ContextMenu key={ref.artifactId+ref.versionId}><ContextMenuTrigger asChild><div className="artifact-attachment-row" onFocusCapture={e=>{if(e.target instanceof HTMLButtonElement)focusTarget.current=e.target}} onContextMenu={e=>{focusTarget.current=e.currentTarget.querySelector('button')}}><TooltipButton tooltip={`Open ${ref.name} — ${v?`version ${v.number}`:'saved version'}.${a?.versions.at(-1)?.id!==ref.versionId&&a?' A newer version is available; use the actions menu to update this attachment.':''}`} type="button" disabled={busy||closing} onClick={actions(ref)[0].action}><File size={17}/><span>{ref.name}<small>{v?`Version ${v.number}`:'Saved version'}{a?.versions.at(-1)?.id!==ref.versionId&&a?' · Update available':''}</small></span></TooltipButton><DropdownMenu><DropdownMenuTrigger asChild><TooltipButton type="button" aria-label={`Actions for attached ${ref.name}`} disabled={busy||closing}><DotsThree size={18}/></TooltipButton></DropdownMenuTrigger><DropdownMenuContent onCloseAutoFocus={restoreFocus}>{actions(ref).map(action=><DropdownMenuItem key={action.label} disabled={busy||closing||action.disabled} onSelect={action.action}>{action.label}</DropdownMenuItem>)}</DropdownMenuContent></DropdownMenu></div></ContextMenuTrigger><ContextMenuContent onCloseAutoFocus={restoreFocus}>{actions(ref).map(action=><ContextMenuItem key={action.label} disabled={busy||closing||action.disabled} onSelect={action.action}>{action.label}</ContextMenuItem>)}</ContextMenuContent></ContextMenu>})}</div>}
 </>
}
