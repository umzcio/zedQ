...
export function VoiceControls({controller,originId,onTranscript,disabled=false,disabledReason,recordLabel}:{controller:VoiceRecorderController;originId:string;onTranscript:(result:VoiceTranscript)=>void;disabled?:boolean;disabledReason?:string;recordLabel?:string}){
 ...
 return <div className="voice-controls" onKeyDown={event=>{if(event.key==='Escape'&&state.phase!=='idle'){event.preventDefault();void controller.cancel()}}}>
  {state.phase==='idle'?<Button type="button" variant={recordLabel?"outline":"ghost"} size={recordLabel?"default":"icon"} tooltip={disabled?(disabledReason??"Dictation is unavailable while this action is in progress"):"Dictate into your draft without sending a message"} aria-label={recordLabel??"Record dictation"} disabled={disabled} onClick={()=>void controller.start(originId,onTranscript)}><Microphone size={18}/>{recordLabel}</Button>:<>
